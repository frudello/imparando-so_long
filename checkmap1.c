...
/*   Updated: 2022/07/22 15:09:47 by frudello         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	walenght(t_map *map_s)
{
	...
	while (map_s->map[++map_s->check.y])
	{
		...
		{
			ft_putstr("Mappa sbagliata\n");
			close_all(map_s->mlx);
		}
	}
}

void	one(t_map *map_s, char *s, char c)
{
	int	i;
...
void	edges(t_map *map_s)
{
	...
	one(map_s, map_s->map[map_s->check.y - 1], '1');
}
