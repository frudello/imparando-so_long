...
/*   Updated: 2022/06/30 19:09:43 by frudello         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	win(t_map *m)
{
	...
	ft_putstr("brava lesbica hai vinto! 8======D");
	exit(0);
}
...
void	move_up(t_map *m)
{
	...
	put_wall(m, m->prighe - 1, m->pcolonne, "three.xpm");
	ft_putnbr(m->mosse += 1);
	ft_putchar('\n');
	m->prighe -= 1;
}
...
void	move_left(t_map *m)
{
	...
	put_wall(m, m->prighe, m->pcolonne-1, "three.xpm");
	ft_putnbr(m->mosse +=1);
	ft_putchar('\n');
	m->pcolonne -= 1;
}
